...
var ClientApi = function(app) {
    ...
    var self = this;
    var NewsClient = require("./newsClient")(app);
    ...
    this.call = function(f, loc, authOnly) {
        ...
        var cb = function(req, res) {
            ...
            if (authOnly) {
                ...
                if (!auth)
                    return self.unauthenticated(req, res);
            }
    ...
    this.getStockList = function(req, res) {
        ...
        // z marketClient nacti data a odesli je
        res.send(marketClient.getStocks());
    };
    ...
    this.getMyStocks = function(req, res) {
        ...
        // z marketClient nacti seznam klientovych akcii
        marketClient.getClientStocks(req.user._id, function(err, data) {
    ...
    this.getPendingStocks = function(req, res) {
        ...
        marketClient.getClientsOrders(req.user._id, function(err, data) {
    ...
    this.setAccountInfo = function(req, res) {
        ...
        if (!validator.isEmail(update.email))
            return res.send({
                error: "Email nemá požadovaný formát."
            });
        ...
        Client.update({
            ...
            if (err)
                return res.send({
                    error: "Při ukládání do databáze došlo k chybě."
                });
    ...
    this.setPassword = function(req, res) {
        ...
        hash(req.form.old, req.user.salt, function(err, hashOld) {
            ...
            if (err)
                return res.send({
                    error: "Při úpravě hesla došlo k chybě."
                });
            ...
            if (hashOld == req.user.password) {
                ...
                return res.send({
                    error: {
                        old: "Staré heslo pro ověření nesouhlasí s heslem v databázi!"
                    }
                });
    ...
    this.buyStock = function(req, res) {
        ...
        var stock = marketClient.getStocksByTicker(info.ticker);

        // otestuje zda je nakup akcii na existujici spolecnost
        if (!stock)
            return res.send({
                error: "Příkaz byl zadán pro neznámou společnost."
            });
        ...
        Client.findOne({
            ...
            if (u.accountBalance < info.price * info.amount)
                return res.send({
                    error: "Na Vašem účtě není dostatek finančních prostředků!"
                });
            ...
            u.save(function(err, o) {
                ...
                if (err)
                    return res.send({
                        error: "Chyba při aktualizaci účtu klienta!"
                    });
                ...
                // odesle prikaz na nakup do marketClient
                marketClient.addBuyOrder(info, function(err, out) {
                    ...
                    res.send({
                        valid: 1,
                        decrBalance: info.price * info.amount
                    });
        ...
        ClientStock
            ...
            .exec(function(err, u) {
                ...
                if (amountLeft > 0)
                    return res.send({
                        valid: 0,
                        error: "Nedostatečné prostředky pro realizaci příkazu!"
                    });
                ...
                // odesle sell prikaz
                marketClient.addSellOrder(info, stockRemove, function(err, out) {
    ...
    function emitInfoToClient(uid) {

        var client = self.getClientById(uid);
        if (!client)
            return false;

        if (ConnectedClientsById[uid].sockets)
            for (var i in ConnectedClientsById[uid].sockets) {

                var s = ConnectedClientsById[uid].sockets[i];
                var out = {
                    accountBalance: parseFloat(client.accountBalance),
                    email: client.email,
                    name: client.name
                };

                // console.log("Posilam klientovi informace o novem stavu.", out);
                s.emit("accountInfo", out);
            }
    }
    ...
    /**
     * Vrati klienta z cache podle ID
     * @param  {String} key klic
     * @return {Object}     Data o klientovi
     */
    this.refreshClientInfo = function(uid) {

        console.log("Updatuji informace o klientovi.");
        var client = self.getClientById(uid);
        ...
        Client.findOne({
            ...
            client.accountBalance = res.accountBalance;
            emitInfoToClient(uid);
        });
    };
    ...
    this.cancelStockOrder = function(req, res) {
        ...
        if (!info.id)
            return res.send({
                valid: 0,
                error: "Při odesílání požadavku došlo k chybě."
            });
        ...
        Order.findOne({
            ...
            if (!data)
                return res.send({
                    valid: 0,
                    error: "Příkaz k zrušení nebyl nalezen."
                });

            // pridame k prikazu ID pod kterym je prikaz veden na burze
            info.orderId = data.orderId;

            // odesle sell prikaz
            marketClient.cancelOrder(info, function(err, out) {
                // pokud doslo k chybe, zaloguj a vrat chybu
                ...
                // uloz prikaz do historie
                ClientHistory.saveEvent(req.user._id, histData, function() {});
            });
        });
    };
    ...
    this.getNews = function(req, res) {
        ...
        res.send(NewsClient.getList());
    };

    this.getStockDetail = function(req, res) {
        var stocks = marketClient.getStocks();

        for (var s in stocks) {
            if (stocks[s].ticker == req.params.ticker)
                return res.send({
                    info: stocks[s]
                });
        }
    ...
    this.doLostPass = function(req, res) {
        ...
        Client.findOne({
            ...
            if (!c)
                return res.render(view_dir + 'lost_pass', {
                    layout: layout,
                    email: req.body.email,
                    msg: "Zadaný email není v naší databázi.",
                ...
                nodemailer.sendMail({
                    ...
                    if (err) {
                        ...
                        return res.render(view_dir + 'lost_pass', {
                            layout: layout,
                            email: req.body.email,
                            msg: "Při odesílání emailu došlo k chybě.",
                            title: "Zapomenuté heslo"
                        });
                    }

                    // vyrendruj stranku s vysledkem
                    res.render(view_dir + 'lost_pass', {
                        layout: layout,
                        email: req.body.email,
                        info: c,
                        title: "Zapomenuté heslo"
    ...
    this.doNewPass = function(req, res) {
        ...
        ClientLostPwd.findOne({
            code: req.params.code,
            finished: null
        }, function(err, changeRequest) {

            if (err)
                return res.render(view_dir + 'new_pass', {
                    layout: layout,
                    msg: "Chyba při práci s databází.",
                    title: "Nové heslo"
                });
            ...
            if (req.body.pass != req.body.pass2)
                return res.render(view_dir + 'new_pass', {
                    layout: layout,
                    msg2: "Zadaná hesla se neshodují.",
                    title: "Nové heslo"
                });
    ...
    Events.on("clientChange", this.refreshClientInfo);
